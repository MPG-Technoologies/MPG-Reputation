...
export function RealtimeStatus({ status }: RealtimeStatusProps) {
  const getStatusConfig = () => {
    switch (status) {
      ...
      case 'RECONNECTING':
        return {
          dotClass: 'bg-amber-400 ring-amber-500/20 animate-pulse motion-reduce:animate-none',
          text: 'Reconnecting…',
      ...
      case 'SYNCING':
        return {
          dotClass: 'bg-blue-400 ring-blue-500/20 animate-pulse motion-reduce:animate-none',
          text: 'Syncing…',
  ...
  return (
    <div
      className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-slate-900 border border-slate-800 text-xs font-medium motion-reduce:transition-none"
      role="status"
      aria-label={config.ariaLabel}
    >
      <span
        className={`w-2 h-2 rounded-full ring-2 transition-colors motion-reduce:transition-none ${config.dotClass}`}
        aria-hidden="true"
      />
